function Carousel(selector) {		
	...
	this.oimgLists = document.querySelector(selector).querySelector('.imgList').getElementsByTagName('li');
...
Carousel.prototype.bindEvent = function() {
	...
	self.oRightBtn.onclick = function(){
	  self.index++;
	  if (self.index >= self.oimgLists.length) {
		self.index = 0;
	  }
	  self.move();
	}
	self.oLeftBtn.onclick = function(){
	  self.index--;
	  if (self.index < 0) {
		self.index = self.oimgLists.length-1;
	  }
	  self.move();
	}
...
Carousel.prototype.move = function() {
  for (var i = 0; i < this.oimgLists.length; i++) {
    this.oimgLists[i].className = '';
  }
  this.oimgLists[this.index].className = 'current';
